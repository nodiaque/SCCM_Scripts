<#	
	...
	===========================================================================
	 ...
	 Created on:   	2023-02-08 07:47
	 Created by:   	Jean-Sébastien Frenette
	 Organization: 	STM
	 Filename:     	SCCM_ChangeTSSecurityScope.ps1
	===========================================================================
	.DESCRIPTION
...
function ajouterScopes
{
	...
	{
		Write-Host "Récupération des task sequences"
		$allTS = Get-CMTaskSequence -fast
		$selectedTS = @($allTS | Select-Object Name, PackageID, SecuredScopeNames | Out-GridView -passthru -Title "Sélectionner les TS à ajouter les scopes.")
		$numApp = $selectedTS.count
		if ($numApp -lt 1)
		{
			Write-Host "Aucune TS sélectionné"
		}
		else
		{
			write-host "$numApp TS sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($ts in $selectedTS)
			{
				$tsName = $ts.Name
				Write-Host "$cpt) $tsName"
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMTaskSequence -Id $ts.PackageID -fast | Add-CMObjectSecurityScope -id $scopeID -ea Stop
...
function retirerScopes
{
	...
	else
	{
		Write-Host "Récupération des task sequences"
		$allTS = Get-CMTaskSequence -fast
		$selectedTS = @($allTS | Select-Object Name, PackageID, SecuredScopeNames | Out-GridView -passthru -Title "Sélectionner les TS à retirer les scopes.")
		$numApp = $selectedTS.count
		if ($numApp -lt 1)
		{
			Write-Host "Aucune TS sélectionné"
		...
		else
		{
			...
			foreach ($ts in $selectedTS)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMTaskSequence -Id $ts.PackageID -fast | Remove-CMObjectSecurityScope -id $scopeID -force -ea Stop
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDq1D2HY8tGy3YW
# ds+uImsQRGnS4PhPEspbyO5FN/WYvaCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# AKBMMBkGCSqGSIb3DQEJAzEMBgorBgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCCV
# k+ojVucngN4cMkWhDIS6/mZOZ1SjmzKKg3gA5SpCoTANBgkqhkiG9w0BAQEFAASC
# AQBZ2dXqws24BCt0crOtora/nubH8zuaq8V4GPGb94S0RTG8869L121vwK8bzHeJ
# jWoZrsyfaOexd+p0/F+JctyUFWNruD4qfevMjpbYby3o97G/y7zQ13w+8EmklF8E
# HmBfy4QKNUkSlFtMIVyhc/nS81gUcaHDIWEHZ7Bf/vNpnqgK8qi+48+NkWF5lbRX
# WFe2wer7AkTDargi0WdPyDJlmQ//qEhfC1Od1MyD/PhbPeb9rAwTGBZUWzn7hoqm
# Iw9v3v79BUpssfYa/V3tlNV0UW21mGI8mM0oOPmJOTslD2m8pndHBHlGAmxtF8vF
# 8PfP5m8aBe1EbYDsKBfQE/oQoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMDgxMjU0MTlaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCCskPxA1KglXfWMEYfOAMisOKkr5N8uBr3FDbaT7pJSXTCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgHm/mI7oVl1qieJIfqtg
# hhf3F5+DU0nEZQ2/XPkYNUKqDg+zDm287pX7+oLcq/VEBi6B1dXyKqCALQgLyc6y
# Dx3WdQtCVNl4Hd1+kC6cs72oTbKGfDtN9PX9dnOrmns5vnFCj3xCSFfHs9+ZCWaN
# XJ9KXerdB7jUERQli+eGgNeKCNz8dD3QM1c7XSrLE7bfn3taiBZbxApNs+WWjVDd
# 28046EKA/2oosBrxku1lrFObX0ZUFYKsGNMxd03+6I5KarFk6KE4O1MVVVsFjExz
# 4fc1ftH5yTJEN4MERvQoW0QvoQFU1/QfNpTDyuISiuBVVVpOc/CK3UUto3Z3KNzF
# gweZ4V6EQQKu+g8A6kZR9rPX9sarfZ59Wd2/0ToRs6LGWac8vmeWC4lMiqvFE95A
# aiU+oex5ZTfXiFc6dF1BmHvDFA/NEGhA/B5qDOS42MmMC7J57JZYd/w/oz7g3gBQ
# T6FouA+loRCFYQHu6JCWWcnYpLGdBBsSel4xNlCJkRrIGg==
