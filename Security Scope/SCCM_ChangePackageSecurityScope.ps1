<#	
	...
	===========================================================================
	 ...
	 Created on:   	2023-01-24 09:51
	 Created by:   	Jean-Sébastien Frenette
	 Organization: 	STM
	 Filename:     	SCCM_ChangePackageSecurityScope.ps1
	===========================================================================
	.DESCRIPTION
...
function ajouterScopes
{
	...
	{
		Write-Host "Récupération des packages"
		$allApp = Get-CMPackage -fast
		$selectedApps = @($allApp | Select-Object Name, PackageID, SecuredScopeNames | Out-GridView -passthru -Title "Sélectionner les packages à ajouter les scopes.")
		$numApp = $selectedApps.count
		if ($numApp -lt 1)
		{
			Write-Host "Aucun package sélectionné"
		}
		else
		{
			write-host "$numApp packages sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($app in $selectedApps)
			{
				$appName = $app.Name
				Write-Host "$cpt) $appName"
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMpackage -Id $app.PackageID -fast | Add-CMObjectSecurityScope -id $scopeID -ea Stop
...
function retirerScopes
{
	...
	else
	{
		Write-Host "Récupération des packages"
		$allApp = Get-CMpackage -fast
		$selectedApps = @($allApp | Select-Object Name, PackageID, SecuredScopeNames | Out-GridView -passthru -Title "Sélectionner les packages à retirer les scopes.")
		$numApp = $selectedApps.count
		if ($numApp -lt 1)
		{
			Write-Host "Aucun package sélectionné"
		...
		else
		{
			...
			foreach ($app in $selectedApps)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMpackage -Id $app.PackageID -fast | Remove-CMObjectSecurityScope -id $scopeID -force -ea Stop
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBtw1T6D5Q0y/x6
# AYz7BLzmjhHvIeE867MQUgxnvFUIVaCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# AKBMMBkGCSqGSIb3DQEJAzEMBgorBgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCCr
# /SFFzaByhkS5gpyTaLmHZU4+i5I5dvxSYnE8XIRtZjANBgkqhkiG9w0BAQEFAASC
# AQB8H59tiPbsTID1mVDkfpXcKqsRkv1deRACwDIrDEwLa6Jpr/ySBHwCnompsvtK
# v+r4R8xadfNxJDFE5Mch89SAWU9uZxk9E0GrlghA8OJoUfAemD7QV6iWJtt/y5JE
# JVRY5lSSZmHmwz0LrydAMPNeY5A2iv5PssKnW0cAq4OMEvdBXMMpPCVgvVPhN3Fb
# e6iYtPqlHUaYzHxmUne4DSepiXfAAO80OipTgh+pW/rr1Hx/4vcyjov2H3dzoD6l
# iHMCy86F3iYm4Y2EwzXBWRdhEbaXTEnCH5LyeLwe1BcDNqaCo+MMRfnsB5n0Aa6V
# UtSEFy97SYRZM1uxIqHG3U6qoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMDgxMjUxNTZaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCDMioTixIC5KtOzohNshRT0j5+7iD2kRpDw74TjbsrXwDCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgLKnsR7bwDu52eCDVFAm
# laAs66y63R3wWCSkld7SbrxL4Xs6/DrueVWUkTBJQ0DmkI76FlcpZVdHGuPPMI/g
# M0hfitOMSlC62+XcH/WV/NPw9Dh+EkreCzzeHbebVYWxqxkx3h08PB4Cn4HTLqmM
# ENBHozA0jKagEj8MaTDpU+evtlEeOUT/Vs+vTXbfCnLxtP5Zt68+BL8jPde+n0Xc
# 59USi6OE9IQZWB8cuqYroigH/MEZxTKBQGoozrDQWlTfO6ZmbuQZ10iPUXQdXWK6
# Nu5k9p97DwvLJXPkcC4X2x9omQ0KhngXQc7+/dRA8v76ATu9U8NOnpRmynFNjKLM
# tdWI2GSBtPMbFvsXpFeSNPXUfncjcnsW2eAiqrzZeab99COwN2ZpRB/D6VMX+6Pn
# FWgqbiUbKc1tpOL5lawvUCLOiIhLgQMSrfRzTZ0POUU9ckvlv4lThRnH3uBnLCW1
# IqGKBjuyFxP6i2zk56n9AVUE4A1RafuyMSJRVDZpw6xdrA==
