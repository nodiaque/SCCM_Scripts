<#	
	...
	===========================================================================
	 ...
	 Created on:   	2023-01-13 13:47
	 Created by:   	Jean-Sébastien Frenette
	 Organization: 	STM
	 Filename:     	SCCM_ChangeCollectionExcludeRule.ps1
	===========================================================================
	.DESCRIPTION
		Enlève les membership exclusions et remplace par un autre
#>
...
function collectionsPostes
{
	...
	$excludeDeviceCollection = @($cols | Out-GridView -passthru -Title "Sélectionner la nouvelle collection ordinateur à être excluse")
	
	$excludecollectionname = $excludeDeviceCollection.Name
	$excludecollectionid = $excludeDeviceCollection.CollectionID
	If ($excludeDeviceCollection.Count -gt 1)
	{
		write-host "Plus de 1 collection a été sélectionné" -ForegroundColor Yellow
		
	}
	else
	{
		If ($excludeDeviceCollection.Count -eq 1)
		{
			write-host "Nom de la collection ordinateur à exclure: $excludecollectionname"
			write-host "ID de la collection à exclure: $excludecollectionid"
		}
		else
		{
			Write-Host "Aucune collection de sélectionner, on retire seulement"
		}
		
		Write-Host "Récupération des règles d'exclusion, ceci prendra un certain temps..."
		$cols = @()
		foreach ($col in $allcols)
		{
			$rules = @(Get-CMDeviceCollectionExcludeMembershipRule -CollectionId $col.CollectionID)
			foreach ($rule in $rules)
			{
				[hashtable]$excluderuleProp = @{
					Name				  = $col.name
				...
				$excludeRule = New-Object -TypeName System.Management.Automation.PSObject -Property $excluderuleProp
				$cols += $excludeRule
			}
			
		}
		write-host "Sélection des règles d'exclusion d'ordinateurs à modifier"
		
		$cols = @($cols | Select-Object Name, CollectionID, ExcludeCollectionID, RuleName, MemberCount)
		$rulestomodify = @($cols | Out-GridView -PassThru -Title "Sélectionner les règles d'exclusion à modifier.")
		...
		foreach ($rule in $rulestomodify)
		{
			...
			try
			{
				If ($excludeDeviceCollection.Count -eq 1)
				{
					Write-Host "$cpt) Ajout de $excludecollectionid"
					Add-CMDeviceCollectionExcludeMembershipRule -CollectionId $rule.collectionid -ExcludeCollectionID $excludecollectionid -ea Stop
				}
				Write-Host "$cpt) Retrait de $($rule.ExcludeCollectionID)"
				Remove-CMDeviceCollectionExcludeMembershipRule -CollectionId $rule.collectionid -ExcludeCollectionID $rule.ExcludeCollectionID -force -ea stop
			}
			catch
			{
...
function collectionsUtilisateurs
{
	...
	$cols = @($allCols | Select-Object Name, CollectionID)
	$excludeDeviceCollection = @($cols | Out-GridView -passthru -Title "Sélectionner la nouvelle collection utilisateur à être excluse")
	
	$excludecollectionname = $excludeDeviceCollection.Name
	$excludecollectionid = $excludeDeviceCollection.CollectionID
	If ($excludeDeviceCollection.Count -ne 1)
	{
	...
	{
		write-host "Nom de la collection utilisateur à excluse: $excludecollectionname"
		write-host "ID de la collection à excluse: $excludecollectionid"
		Write-Host "Récupération des règles d'exclusion, ceci prendra un certain temps..."
		$cols = @()
		foreach ($col in $allcols)
		{
			$rules = @(Get-CMUserCollectionExcludeMembershipRule -CollectionId $col.CollectionID)
			foreach ($rule in $rules)
			{
				[hashtable]$excluderuleProp = @{
					Name				  = $col.name
		...
		write-host "Sélection des règles d'exclusion d'utilisateur à modifier"
		...
		foreach ($rule in $rulestomodify)
		{
			...
			{
				Write-Host "$cpt) Ajout de $excludecollectionid"
				Add-CMUserCollectionExcludeMembershipRule -CollectionId $rule.collectionid -ExcludeCollectionID $excludecollectionid -ea Stop
				Write-Host "$cpt) Retrait de $($rule.ExcludeCollectionID)"
				Remove-CMUserCollectionExcludeMembershipRule -CollectionId $rule.collectionid -ExcludeCollectionID $rule.ExcludeCollectionID -force -ea stop
			}
			catch
			{
				Write-Host "Erreur" -ForegroundColor Red
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC5xoprhQSz5ZDY
# V4JPivgtI1ASnN0FHSewPtRMWxWLHqCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# AKBMMBkGCSqGSIb3DQEJAzEMBgorBgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCBR
# 1N9QpZovkemLYQlxoBXQVUrlG3oKYa5Xyv3qlWqQ6zANBgkqhkiG9w0BAQEFAASC
# AQCTbmQahzUiXctSnl6SS955VEsCSfFDEHDDok79fAmgM0D/sPMsjL3e1Vihp1os
# STyn1glGvUWRmAIswjDP2B2pw8z4tIZ3G6BlKEe6LodHHA6lk6A3YMOfv9kpvHP2
# U+tkYJICrwYpY9rC3v0hMBAj3mn9MmSITYuuyk89XP63h0CdDNv8Z4bKlwCxSKOs
# cL5EHJLtvS0cAfb1rGvO0a7QMINs9t2fPpb+Q+fZzrD9o9YlcxfUwXbv5VOgpKVz
# 6AN+8scY713Dz3f2exmensyGHAkh8sKUmO5KxpvJ+WkL2h1GjSSGOLmlXmwqg5PG
# R8CTyb+VKZfwpkOIndXuk6AEoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAxMjUxNTA2MjdaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCB/KKKuyEEN9J+XkNp1L8+XhZsLo4WALx9Im+q9PIDDvzCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgANRv/mUZ6uLURuck6Tr
# Ube+n9sjaJBqnyNdEmjTxXgsegR6L0cTUxPrMmo0ug9uO2E//54WM3aWp6JIMkgY
# TOXrm4y0vlnOiji/tx47Jf+YnuLFeq1se0Eq+iZO2QdwJ8ONsQojx9e13LqYl4mH
# dQa0jIuJzqv7aJCuY0MHb8uoxmEQ5HegTJ9eUYGsdUWkAGz8U8eNnEY4qUYRJB87
# ShlQF4COAGSdA46q0srEKapJJLE8yHmudXxeth8L7jDdSWiluHFbL5DMHKWl9/92
# qUVgFIwy84OjFIWX12WKfKmr/v9559nDlJBd2rRXtcu4AKK8QoJ5q80oJpSJe6Op
# /vAeLSeHPKy2ChwmV0S0nm1gimnHxBclT38gBqbo/VlG1SjK5BOkXSE9cAmhgfxM
# 91/ZwjQzHxSAOoJA6PeLWZ6qMp1RjjPXsxxzt7LuXvz0BP8XUr0FD4BfHA8+AOWf
# ZTAn1Oj/F2/LCKyt7kRNLjev3x+19vuiKvM/tm9UlCtkPg==
