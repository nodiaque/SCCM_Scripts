<#	
	...
	===========================================================================
	 ...
	 Organization: 	STM
	 Filename:     	SCCM_ChangeBootImageSecurityScope.ps1
	===========================================================================
	.DESCRIPTION
		Ajoute et retire les security scopes sélectionnés des BootImage sélectionnés
#>
...
function ajouterScopes
{
	...
	{
		Write-Host "Récupération des BootImage"
		$allBootImage = Get-CMBootImage
		$selectedBootImage = @($allBootImage | Select-Object  Name, PackageID, SecuredScopeNames, Description | Out-GridView -passthru -Title "Sélectionner les BootImage à ajouter les scopes.")
		$numBootImage = $selectedBootImage.count
		if ($numBootImage -lt 1)
		{
			Write-Host "Aucun BootImage sélectionné"
		}
		else
		{
			write-host "$numBootImage BootImage sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($BootImage in $selectedBootImage)
			{
				$BootImageName = $BootImage.Name
				Write-Host "$cpt) $BootImageName"
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMBootImage -Id $BootImage.PackageID | Add-CMObjectSecurityScope -id $scopeID -ea Stop
...
function retirerScopes
{
	...
	{
		Write-Host "Récupération des BootImages"
		$allBootImage = Get-CMBootImage
		$selectedBootImage = @($allBootImage | Select-Object Name, PackageID, SecuredScopeNames, Description | Out-GridView -passthru -Title "Sélectionner les BootImages à retirer les scopes.")
		$numBootImage = $selectedBootImage.count
		if ($numBootImage -lt 1)
		{
			Write-Host "Aucun BootImage sélectionné"
		...
		{
			write-host "$numBootImage BootImages sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($BootImage in $selectedBootImage)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMBootImage -Id $BootImage.PackageID | Remove-CMObjectSecurityScope -id $scopeID -force -ea Stop
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAnVgRKAcyrSU4K
# 4i+1Yarn+4fxGTifLlbv4Y/Fixvoy6CCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# AKBMMBkGCSqGSIb3DQEJAzEMBgorBgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCD2
# gL3F0WbHsgNApBp/3BkwOGk0bIi7ZbJzKOxTcSV/ujANBgkqhkiG9w0BAQEFAASC
# AQCACok9IQhMX1i9sihQXW4UY7HX3u1cOyJNZxE94MlB43WJnVNvV/8bynrHIEx3
# 6R8dMP0af99O/5w4edFFBloC8ldH1DGPmira3cNs7FbBGRi1H8UtxYzGr9EiG2aI
# PiQVRXbKCELrzgFMU4kaYSOJP1Jpfm6Hpkm00RKeYOD+fbylGemR8DcxtRxpwvRh
# HFup4Dto/rqkxctlp25dWAJg7CW4f50pYeDxJ2saTWDxRR2UgprWm9vyZgMD5rqa
# /Hyi11gLTaZ8lv0zzIzPwdf1WCasF3L0udvXfyjkhsWJLkMdvXdYF//bmPcjcD0s
# zcyUQA0ZFm2kEO+PX3DF+V3boYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMTAxNjMyMjBaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCApbe9phT2I/98OAA3XI1RchUy/fYJzvObB3KH/DvexWjCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgGhRwtpxTlIdgx5eDTSm
# LyUfm23isiMshJyMVSv9AGjsFM1OH82TSx2Qu/v/Y5VX/CmyVjMWpEcXc5p0BD6L
# kXL1niI59syfQ65TkFd4LlH1rU+7sXbRoI3vfkRIGW6rRJo1xHLtRTbTtopU/eGK
# dd86qFBPzl1ni23GcqEL8Kv4B4/IuEP+XopSwXnW7Vsz6YyqfaB+22M+1pdM7/Ry
# xXzih5gQTV2tEmUW+h+W6Iwv9oZ7OaISp1csuPLtRZLUY77AAvwKP/DMt74bXreb
# tsXNc1TUlcYIsWKcGmMIuBB+dR995I8F2Cal00B3JNrKQxl+xPW3RRU5qJ1U7d3X
# fPOhbChB9QSF2ePIHhmMJL4oP55WlQPnh65z8hK33lnojjbW0oK2H3OZsGwIKa/E
# 5UqRPx+Qce8fVIxZ39YPBSoE5RWRM53UFXn8eKMQb9bkumo71oU8nLwxFIyubNqD
# mqSAJjMIILOefLy8jy+J93G1VcMO3TPKUprjCOxA0gFUdw==
