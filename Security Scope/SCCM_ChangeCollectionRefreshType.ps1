<#	
	...
	===========================================================================
	 ...
	 Created on:   	2023-01-20 07:50
	 Created by:   	Jean-Sébastien Frenette
	 Organization: 	STM
	 Filename:     	SCCM_ChangeCollectionRefreshType.ps1
	===========================================================================
	.DESCRIPTION
		Permet de modifier le refresh type des collections
		Si l'option cédule est prise, une question concernant l'heure de départ sera demandé. Les minutes et secondes
		sont aléatoire entre 0-60 pour chaque déploiement.
		Standard mis en place des cédules:
		- 4-5h: Disponible par groupe AD
		- 5-6h: Désinstallation
		- 6-7h: Mise à jour
#>

$refreshTypeList = New-Object System.Collections.ArrayList
$noRefresh = @{
	Name = "None"
	RefreshID = 0
}
...
[void]$refreshTypeList.add(([pscustomobject]$noRefresh))
[void]$refreshTypeList.add(([pscustomobject]$manRefresh))
[void]$refreshTypeList.add(([pscustomobject]$cedQuotRefresh))
[void]$refreshTypeList.add(([pscustomobject]$ced7jrRefresh))
[void]$refreshTypeList.add(([pscustomobject]$cedHourRefresh))
[void]$refreshTypeList.add(([pscustomobject]$incRefresh))
[void]$refreshTypeList.add(([pscustomobject]$bothRefresh))
...
function collectionsUtilisateurs
{
	...
	$refreshType = @($refreshTypeList | Out-GridView -passthru -Title "Sélectionner le nouveau refresh type.")
	If ($refreshType.Count -ne 1)
	{
		write-host "Aucun refresh type sélectionner" -ForegroundColor Yellow
		
	}
	else
	{
		write-host "Nouveau refresh type: $($refreshType.Name)"
		write-host "Récupération des collections utilisateurs"
		$allcols = @(Get-CMUserCollection)
		$cols = @($allcols | Select-Object Name, CollectionID, RefreshType, LimitToCollectionName, LimitToCollectionID, MemberCount, ServiceWindowsCount)
		$Collectionstomodify = @($cols | Out-GridView -PassThru -Title "Sélectionner les collections à modifier le refresh type (0-None, 1-Manual, 2-Cédule, 4-Incremental, 6-Both)")
		$num = $Collectionstomodify.count
		Write-Host "$num collections à modifier"
		$cpt = 1
		Write-Host "Début du traitement"
		
		
		if ($refreshType.RefreshID -eq 2 -or $refreshType.RefreshID -eq 6)
		{
			$refreshHour = Read-Host -Prompt "Entrer l'heure du début (chiffre uniquement)."
			if ($refreshHour.Trim().Length -lt 0)
			{
				Write-Host "Entrée invalide, terminaison"
				exit
			}
		}
		...
		foreach ($Collection in $Collectionstomodify)
		{
			...
			write-host "$cpt) $colid - $colName"
			$cpt++
			try
			{
				if ($refreshType.RefreshID -eq 2 -or $refreshType.RefreshID -eq 6)
				{
					$cedDate = (Get-Date).Date
					$cedDate = $cedDate.AddHours($refreshHour)
					$cedDate = $cedDate.AddMinutes((Get-Random -min 0 -max 60))
					$cedDate = $cedDate.AddSeconds((Get-Random -min 0 -max 60))
					Write-Host "La cédule est $cedDate tous les $($refreshType.RecurCount) $($refreshType.RecurInterval)"
					$schedule = New-CMSchedule -RecurCount $refreshType.RecurCount -RecurInterval $refreshType.RecurInterval -IsUtc -Start $cedDate
					Set-CMCollection -CollectionId $colID -RefreshType $refreshType.RefreshID -RefreshSchedule $schedule
				}
				else
				{
					Set-CMCollection -CollectionId $colID -RefreshType $refreshType.RefreshID
				}
...
function collectionsPostes
{
	...
	{
		write-host "Nouveau refresh type: $($refreshType.Name)"
		write-host "Récupérations des collections machines"
		$allcols = @(Get-CMDeviceCollection)
		$cols = @($allcols | Select-Object Name, CollectionID, LimitToCollectionName, LimitToCollectionID, MemberCount, RefreshType, ServiceWindowsCount)
		$Collectionstomodify = @($cols | Out-GridView -PassThru -Title "Sélectionner les collections à modifier le refresh type (0-None, 1-Manual, 2-Cédule, 4-Incremental, 6-Both)")
		$num = $Collectionstomodify.count
		Write-Host "$num collections à modifier"
		$cpt = 1
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCNbKheVYTGCbpb
# 4qMwf+LrvIJe132PtRt9HEEnEucG8KCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# 5qu//Tnx8dp7s7zC8jc3eDo0FB6zFEmx7YubIdopEDANBgkqhkiG9w0BAQEFAASC
# AQBrahOxR29a78PJQdEH7i5d0jrFxXbi4hlu+4EeTcHBOEFE000W/cI1CSSzSqO9
# yK08OidCNmqlzNbklFgbqnFTTA2ryn3cgydmlXV6zFi48kdPv1ZTG4L9QFuQBT/l
# ssdNHEDpYp7vuvsNWYyrcYcMqYnRvmNXRPeDpQn9lpl+wOHv13PF853Jl7mK2O4R
# VRKsIrke30BpZEnpj/GJ6GIkJBdyKlfhR4T0MdLOH+Umef+rDiDMnqbPgadwvF2U
# 6Ov7NZxEtk+rLRxsB8USU9JX6LgYFDn0cbXfatKLCeTeJagiMiFR49+MYDX05xQW
# 9f7X9+iLUkmr1mHnTAiI94lkoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAxMjQxMzUyMTZaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCDQoFRR2nfVJNIzVG7VCCnS4eGC1wV2w9dzwOxvm/LqXzCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgDmkLlTeGJdESMj7Pj2V
# sZgUzXQ9ngSIN+VawBS6awdTZeT65PUu8IxTkc71L0jIC1x5PypHGvm9HLPM4bGe
# kgghV7JTtswixNKwOLvURKzMTRw0QPOgak8NIuqYRhQryP/wj+D284qjwmQ4XC6Q
# d77fRyseHOPLwiYBDDPg1ttKCFEEc4Su6tyYjNDrashFkW2VflX8F8s3LrAtErh2
# f00+RoiS1dnzwgocgb3poP2ASmfM3QWmh8Ya/Cd7hcMR4K5PZJ1cRvctK+3KxWyI
# PGws+lf25tD5PQ8QeMXmXfXdEaCH1+VIA0KaLdZiaStTRWZReJBFoRwq6+mlPS1E
# U1YubkDXkTzVwogWJB15N0NnjfWmI7gREIE0/sLVCxaFyeZDumd/J4wRFe5MFMfg
# /W9GOA+RTK3iVvHyVBTMbVXtk0j+zqtJgrqkY0Z0t9NKgEBpMTK7TdcnSXK7QYuO
# 4cO/qow8QNoVVgHceAeTN5DcqHs7EW34myGOhwKc8pcMqQ==
