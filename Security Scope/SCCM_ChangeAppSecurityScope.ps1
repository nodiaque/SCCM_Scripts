<#	
	...
	===========================================================================
	 ...
	 Created on:   	2023-01-24 09:41
	 Created by:   	Jean-Sébastien Frenette
	 Organization: 	STM
	 Filename:     	SCCM_ChangeAppSecurityScope.ps1
	===========================================================================
	.DESCRIPTION
...
function ajouterScopes
{
	...
	{
		Write-Host "Récupération des applications"
		$allApp = Get-CMApplication -fast
		$selectedApps = @($allApp | Select-Object LocalizedDisplayName, CI_ID, SecuredScopeNames | Out-GridView -passthru -Title "Sélectionner les applications à ajouter les scopes.")
		$numApp = $selectedApps.count
		if ($numApp -lt 1)
		{
			Write-Host "Aucun application sélectionné"
		}
		else
		{
			write-host "$numApp applications sélectionnés"
			$cpt = 1
			...
			foreach ($app in $selectedApps)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMApplication -Id $app.CI_ID -Fast | Add-CMObjectSecurityScope -id $scopeID -ea Stop
...
function retirerScopes
{
	...
	else
	{
		Write-Host "Récupération des applications"
		$allApp = Get-CMApplication -fast
		$selectedApps = @($allApp | Select-Object LocalizedDisplayName, CI_ID, SecuredScopeNames | Out-GridView -passthru -Title "Sélectionner les applications à retirer les scopes.")
		$numApp = $selectedApps.count
		if ($numApp -lt 1)
		{
			Write-Host "Aucun application sélectionné"
		...
		else
		{
			...
			foreach ($app in $selectedApps)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMApplication -Id $app.CI_ID -Fast | Remove-CMObjectSecurityScope -id $scopeID -force -ea Stop
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBbzKtsLJA7CCVy
# Bx01SNZOZZ9kCzsPECL75SEAUk0CxaCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# GwMCFsAx7uM1FRtwfmIA6pdrBucYGnvju6p2XG+54jANBgkqhkiG9w0BAQEFAASC
# AQBzsO857ZHxaxU/oWY6DeBB5De4IcAVpHO0jMpaSqqFDi9RuetBTjPm9Ibp35SL
# oSgqgapzUj0gCPSWpDKPook6DVaVsxPO3r/QpT/STNFIAKYJmdQHJuwN/RtcFHi4
# CaHg1ku1Krric6/e6D1scPM8qe0k5PAVS06+3JVm/bPNJIhh0swOx1PzSaKZCNhF
# 0gD1XhENq4k0fkSLwiv+5U4LFsVKrKRBg02k46N01FiAAIgrhFh16F6RQqDG/Zh2
# 8hMzWlPV8I1PY3RJ8ciVFdGZhXDE38rygTUOXZDgceXSP/ek7yXTvxvPkXKBSH2J
# 3myhS7JUEMrNtdyKsXePeAv4oYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMDkxMzQxMzJaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCAi40f2MMv1s2ZNRiskxH5XTGKYvAvM742QH0YB2AELizCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgCmw4nsQqyLYpL4PRf9c
# umBeGzDWUGkLiiHjm1NgKxhHWfI6jjqXU1Js40LqCym4+G9zKPRnM8tqXd+D/zNm
# RZ1kbQTz9405cc4xOd/1XHJxt8IQQ62sudKb1nu5Cs+ROxDMKmuggIBoXEjImTAf
# 5Ir7gLfzE7FvVgV12fwAbksui3Rd1sjG2uDZkIrMjVTBOoXpkaJQ94Dg5OugiuUx
# 2BhXiGo+7t6wNtcdRoMOAwSB9DUFrioe5/jJhTnYHC0IvTYJcV0xuAP0qZQgTs0X
# 0VajEW0r0i4m1YS4yQCnUw3kacD2mQIkiv5d+jcR1La6jI6aV8Jal9Gbee3kPlHN
# D2L6vcMSDT/k9YAurTjqZWo8rpdzn5w1aekV7nPEi02tdDnoQ1FKgHY7yexW4DMA
# 8KenYQIJ42kNa16p7CWf4afD4RLhy/PLau5RLjkGzJ/GMmQletrgCO/GedC9qmry
# 48OcKFkUnAUX8MFq2VeeQyKM4FLLqpswirRq6dgHGHTQTw==
