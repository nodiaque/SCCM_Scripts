<#	
	...
	===========================================================================
	 ...
	 Organization: 	STM
	 Filename:     	SCCM_ChangeBaselineObjectSecurityScope.ps1
	===========================================================================
	.DESCRIPTION
		Ajoute et retire les security scopes sélectionnés des baseline objets sélectionnés
#>
...
function ajouterScopes
{
	...
	{
		Write-Host "Récupération des baseline"
		$allBaseline = Get-CMBaseline -fast
		$selectedBaseline = @($allBaseline | Select-Object LocalizedDisplayName, CI_ID, ObjectPath, SecuredScopeNames | Out-GridView -passthru -Title "Sélectionner les baseline à ajouter les scopes.")
		$numBaseline = $selectedBaseline.count
		if ($numBaseline -lt 1)
		{
			Write-Host "Aucun baseline sélectionné"
		}
		else
		{
			write-host "$numBaseline baseline sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($baseline in $selectedBaseline)
			{
				$baselineName = $baseline.LocalizedDisplayName
				Write-Host "$cpt) $baselineName"
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMBaseline -Id $baseline.CI_ID -Fast | Add-CMObjectSecurityScope -id $scopeID -ea Stop
...
function retirerScopes
{
	...
	{
		Write-Host "Récupération des baselines"
		$allBaseline = Get-CMBaseline -fast
		$selectedBaseline = @($allBaseline | Select-Object LocalizedDisplayName, CI_ID, ObjectPath, SecuredScopeNames | Out-GridView -passthru -Title "Sélectionner les baselines à retirer les scopes.")
		$numBaseline = $selectedBaseline.count
		if ($numBaseline -lt 1)
		{
			Write-Host "Aucun baseline sélectionné"
		...
		{
			write-host "$numBaseline baselines sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($baseline in $selectedBaseline)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMBaseline -Id $baseline.CI_ID -Fast | Remove-CMObjectSecurityScope -id $scopeID -force -ea Stop
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB02aaM+YFGfJGe
# AuTThTBkHpzkdFHkQ2Cg6kbXau7+2qCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# GaeLw7/nYExdfUuhmHj4YInvTFoBqpVXbt/YBOzXsDANBgkqhkiG9w0BAQEFAASC
# AQCjR6zoM9jXqBw9IAm86nfoSuSQG0SWjL7kjUlKBc22GRuPDU+A9NICsUCdGDQD
# TWoIE09SP1VuqRB4fLe3sWpIz/TfOBVIqKppWsEg+FhSx1WDCEfhtHBkqsgGd4JQ
# CL2+7OtubNDxmAah0lxd1zUhwQsaHLBUnMM1OYPwwBbivehR1vvHDCgTfQzHqvrN
# ujowQHTKPZSAmZfe3lng0vlT8qJ1Iq1F8KYgac4nQ6jyPYM1peX/pbRVj/UDgMx3
# nEU1hQwetN3yK/ut6z//rmLbZJcJVkxt/CdwRxxJ3TSUBZ3LGp4KCe+yBkZhol27
# 2QUsXhRTfMiyjWFdYzsCCrdsoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMTAxNDQxNTlaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCBFt+Ae3ca4JnBjjrHk/2TznGaxCAwFUY0Huo5F0qHwdjCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgFMRDKsu0o2LiDiw3lhy
# 8yR2WEqAdzTm4dxAMnimEQzzZgOkJwgv8j6ZzrMI4ASID4lLeTTqH6ouixB6E1dk
# q5mnIZoJWZcp7Ff3CFmZ92tLTY3RqcnL2AAbMK9cL8whAJtb0QVWnfVrwcsx1ke9
# csOggD/CsSHETxLYaIs0m1YMhxgvxqV8JtFUX0lylXBW7RVT6EZRvM89HCPd6FFL
# c7QjtVvEzaA7KR10keTUGQfLYLMaREETtUVfwdZQZQZJ/holfR9Hmq22nXZ3aYTf
# tZtrlvPt4+Xa2qG3AsRFWPodgbyL1tAefwqWVYx10YksFmShzyt/bpgOkiWH1FBz
# h5kjvGU18nH0Z2fiYSY2uHh3ebuvTO1i7Px0rKJ4J6HcAU3GlaxgqtGgei7M7P4Z
# yu5QWBOiw90Ar/hAaKLeaiskd3dNfyRpmLQe9RTpel36E5Qc0unR0QXL8WpW2mQW
# zEVRyL1tMKgmeeRQ8J11kxTos1Gss02SltEavM/uFiqk6A==
