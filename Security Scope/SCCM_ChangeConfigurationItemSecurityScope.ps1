<#	
	...
	===========================================================================
	 ...
	 Organization: 	STM
	 Filename:     	SCCM_ChangeConfigurationItemSecurityScope.ps1
	===========================================================================
	.DESCRIPTION
		Ajoute et retire les security scopes sélectionnés des configuration item sélectionnés
#>
...
function ajouterScopes
{
	...
	{
		Write-Host "Récupération des ConfigurationItem"
		$allConfigurationItem = Get-CMConfigurationItem -fast
		$selectedConfigurationItem = @($allConfigurationItem | Select-Object LocalizedDisplayName, CI_ID, SecuredScopeNames, LocalizedDescription | Out-GridView -passthru -Title "Sélectionner les Configuration Item à ajouter les scopes.")
		$numConfigurationItem = $selectedConfigurationItem.count
		if ($numConfigurationItem -lt 1)
		{
			Write-Host "Aucun Configuration Item sélectionné"
		}
		else
		{
			write-host "$numConfigurationItem Configuration Item sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($ConfigurationItem in $selectedConfigurationItem)
			{
				$ConfigurationItemName = $ConfigurationItem.LocalizedDisplayName
				Write-Host "$cpt) $ConfigurationItemName"
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMConfigurationItem -Id $ConfigurationItem.CI_ID -Fast | Add-CMObjectSecurityScope -id $scopeID -ea Stop
...
function retirerScopes
{
	...
	{
		Write-Host "Récupération des Configuration Items"
		$allConfigurationItem = Get-CMConfigurationItem -fast
		$selectedConfigurationItem = @($allConfigurationItem | Select-Object LocalizedDisplayName, CI_ID, SecuredScopeNames, LocalizedDescription | Out-GridView -passthru -Title "Sélectionner les Configuration Items à retirer les scopes.")
		$numConfigurationItem = $selectedConfigurationItem.count
		if ($numConfigurationItem -lt 1)
		{
			Write-Host "Aucun Configuration Item sélectionné"
		...
		{
			write-host "$numConfigurationItem Configuration Items sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($ConfigurationItem in $selectedConfigurationItem)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMConfigurationItem -Id $ConfigurationItem.CI_ID -Fast | Remove-CMObjectSecurityScope -id $scopeID -force -ea Stop
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDD2waKQExJYYAQ
# UA09tEHiBh5vNrWZtkRPEWMHLFBvqKCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# M7cvc9B1qTNmjahlTNn0GX9i3zZfaJP708UadGVvTDANBgkqhkiG9w0BAQEFAASC
# AQCbw3xc2LMZOUhQ0Ov+APdjYQw1rNpz80GwKoMwgOMwnVcJxfOnDeV5/FaBqDHM
# J3l7S3sOjkADqX0/naPIJ/AsHVIkneF4KSkLxyJr42wc56k1ezK+izdXbTqrvEck
# pGH848R0v5rwyyNHS8p+n/8plokotNOlaW2i5z7+fiVIjTeO2uliKGG7pypUKNZC
# v6zYWOuBDdKWPeBr8Q1VAxAgu4kjZ1Ls7fJdGswT3wFUzGRR777dYN7iDRD+rMWd
# f2IJ6QfRUwCQNg+ZAd4tus7ULItJL8IXzYBvz+/K7Ny/Xv/L+vsBiaNUg1aYABOS
# 3qbh8nu1W5EVk7cvjkoGcYZIoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMTAxNTA2NDZaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCBASl5t9FuqwaUdw64CHPKLPlLXdxb09AVKNav5AAYBqzCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgDpIHbjD2Bc/Jpi55Pd+
# 40uZ7WRZupnHjV4AfwYH4qKUCZcxhpARWqXtWCoJi9ud0lmgOnybSse3csIKTxJ1
# LY9cD8vKtbpiF1/KtGZF9gp3i9BVbnghT2SH6PJt2rha+NirRwsZLQ3RZStRt1Z+
# 6hz0ru9mefvyfF4rVASGbOEkcXW2w+EmrFoT/8Uo5Jai7bRu3Wfj/pUcUXm3gCRE
# Vq4jWC0BWO0/6u5zSktwem2VKIjo+hY/VqnOuTHK48eciJ7v5Bx2UyxBk50R+xKR
# qMLgxgdn9TjDtU5gjf0QfSaGP5WbQSU8svDcwgPBkzQgtJ595CaTSQNDVmGSxNWW
# uY30mM74+gZwSnG0karv8ow3P4bkJZVf+n1ckhwn7xyEnWODEBeDQfVXhDuruB6P
# MacE+IqOE+2duQ68M09XfR3yuQwNmjnFhJ+gVm1ghmKeZBEZEkjuGpkGlkXBWSZa
# 8ufaKaptscfJVOghCVwAAUmonnRZ64w6KYsfJ+N4J8U+2A==
