<#	
	...
	===========================================================================
	 ...
	 Organization: 	STM
	 Filename:     	SCCM_ChangeLimitingCollection.ps1
	===========================================================================
	.DESCRIPTION
		Modifie la collection limite des colections sélectionnés par celle choisi
#>
...
function collectionsUtilisateurs
{
	...
	$col = @($allCols | Select-Object Name, CollectionID)
	$limitingcollection = @($col | Out-GridView -passthru -Title "Sélectionner la nouvelle collection de limite.")
	
	#$limitingcollection = @(Get-CMDeviceCollection | Select-Object Name, CollectionID | Out-GridView -passthru -Title "Select new Limiting Device Collection, Use The ENTER Key or Mouse \ OK Button.")
	$limitingcollectionname = $limitingcollection.Name
	$limitingcollectionid = $limitingcollection.CollectionID
	If ($limitingcollection.Count -ne 1)
	{
		write-host "Aucune ou plus de 1 collection de sélectionné" -ForegroundColor Yellow
		
	}
	else
	{
		write-host "Nom de la nouvelle collection de limite: $limitingcollectionname"
		write-host "ID de la nouvelle colletion de limite: $limitingcollectionid"
		write-host "Récupérations des collections utilisateurs"
		...
		$Collectionstomodify = @($cols | Out-GridView -PassThru -Title "Sélectionner les collections à modifier")
		...
		foreach ($Collection in $Collectionstomodify)
		{
			...
			try
			{
				Set-CMCollection -CollectionId $colID -LimitingCollectionId $limitingcollectionid
			}
			catch
			{
...
function collectionsPostes
{
	...
	If ($limitingcollection.Count -ne 1)
	{
		write-host "Aucune ou plus de 1 collection de sélectionner" -ForegroundColor Yellow
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAv0rbsAg3/QLgR
# oQfSjx4Dthvd7xwbTUjoC0OSM68kp6CCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# AKBMMBkGCSqGSIb3DQEJAzEMBgorBgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCCZ
# bSO00aOayCiFfn6pGgdftP8QZkComspvR4YZjm0b2DANBgkqhkiG9w0BAQEFAASC
# AQBi+ZEzpsUnPccX8Z7tT3kjxCQ7HbUU75hjImXZEu+F/NbiP25I6osRwVNyUQDL
# /KCE6UINHvsbGMk6vtK2CZTzmhJ1jGL0Bc7Ma6RZZG6D6HY6nrA1Uo4EXoO61pCX
# dWWTrEhJ54MOpsl5KBHu8/nKP/hmFaHYnKzLAFTDbtc2egyxMmSs2XTtVZymxOvY
# astqSWlYJBDsS2I4UNOohgumadPNfZO6POm4KPcfktgD/hKIQ+A0ViGDUTmKLaFZ
# sZKg2ohIxrne4LbQuYwIJuusmKjPFqU1YckO/PFL9asP8dP7rVjNjcZgYqXJFnfJ
# Dz2p1+wqJ3aTvxNfrVq3R6efoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAxMjAxMzM1NTBaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCDY0WbzIbX4UJUwDINb4ZYclpsPXKvFYpcL3R/iy7P4fzCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgDKPPNxrM4mzS2EmUBvR
# Q8qhzNTuCxgOkcALrfVOvXnn+PlMMpXnq64nMv2FGR5nqHlb0fxzWTmdyfIgHGzq
# tmHu7hP017SAFoD9ZunrX8zhd1C2q+fL0XWZICoWWU226GU8TI8zKCZOBg70WXlA
# AyTBxLQ1jw41L6z97oe8llkpn28Q8WT1Tz33j43lNqm5K4Ev2+ytRBLK2+y25rio
# fuIweCszSxS+GNFl2exTgThOW9/WaQ+A3Sbgl6T+2NjDciJ+qYm9uwdYr13Nv7YL
# EvBB9tqLPPO3UZbsaePjMorByviCJK130YpVeQUDyOYY3KtYDeseYWLYFYbD1i6Z
# WyfDJjsZfJTx19b02kOzVZSAyiNhUcN4fea6s1s0Q59QoBFLCtKQ5zVS4mz17eRm
# 2Pg2IIDq0dhul7g1cgCAzLKdi0mpLGgRs5nl0eZCoTvFOPAl8HiJIVpH+LxkqgHS
# WlQyl8/PrRrSEJHPXBAQsRUteqab2IX7nYUk4d+xmWxKUQ==
