<#	
	...
	===========================================================================
	 ...
	 Created on:   	2023-01-16 09:01
	 Created by:   	Jean-Sébastien Frenette
	 Organization: 	STM
	 Filename:     	SCCM_ChangeOSImageInstallerSecurityScope.ps1
	===========================================================================
	.DESCRIPTION
		Ajoute et retire les security scopes sélectionnés des OperatingSystemInstaller sélectionnés
#>
...
function ajouterScopes
{
	Write-Host "Récupération des security scope"
	$securityscope = Get-CMSecurityScope
	$selectedScopes = @($securityscope | Select-Object CategoryName, CategoryID, NumberOfAdmins, NumberOfObjects, CategoryDescription| Out-GridView -passthru -Title "Sélectionner les security scopes à ajouter.")
	
	if ($selectedScopes.count -lt 1)
	{
		Write-Host "Aucun security scope sélectionné"
	}
	else
	{
		Write-Host "Récupération des OperatingSystemInstaller"
		$allOperatingSystemInstaller = Get-CMOperatingSystemInstaller
		$selectedOperatingSystemInstaller = @($allOperatingSystemInstaller | Select-Object  Name, PackageID, SecuredScopeNames, Description | Out-GridView -passthru -Title "Sélectionner les OperatingSystemInstaller à ajouter les scopes.")
		$numOperatingSystemInstaller = $selectedOperatingSystemInstaller.count
		if ($numOperatingSystemInstaller -lt 1)
		{
			Write-Host "Aucun OperatingSystemInstaller sélectionné"
		}
		else
		{
			write-host "$numOperatingSystemInstaller OperatingSystemInstaller sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($OperatingSystemInstaller in $selectedOperatingSystemInstaller)
			{
				$OperatingSystemInstallerName = $OperatingSystemInstaller.Name
				Write-Host "$cpt) $OperatingSystemInstallerName"
				foreach ($secScope in $selectedScopes)
				{
					$scopeName = $secScope.CategoryName
					$scopeID = $secScope.CategoryID
					Write-Host "$cpt) Ajout du scope $scopeName"
					try
					{
						Get-CMOperatingSystemInstaller -Id $OperatingSystemInstaller.PackageID | Add-CMObjectSecurityScope -id $scopeID -ea Stop
						Write-Host "$cpt) Scope ajouté"
					}
					catch
					{
...
function retirerScopes
{
	Write-Host "Récupération des security scope"
	$securityscope = Get-CMSecurityScope
	$selectedScopes = @($securityscope | Select-Object CategoryName, CategoryID, NumberOfAdmins, NumberOfObjects, CategoryDescription | Out-GridView -passthru -Title "Sélectionner les security scopes à retirer.")
	...
	{
		Write-Host "Récupération des OperatingSystemInstallers"
		$allOperatingSystemInstaller = Get-CMOperatingSystemInstaller
		$selectedOperatingSystemInstaller = @($allOperatingSystemInstaller | Select-Object Name, PackageID, SecuredScopeNames, Description | Out-GridView -passthru -Title "Sélectionner les OperatingSystemInstallers à retirer les scopes.")
		$numOperatingSystemInstaller = $selectedOperatingSystemInstaller.count
		if ($numOperatingSystemInstaller -lt 1)
		{
			Write-Host "Aucun OperatingSystemInstaller sélectionné"
		...
		{
			write-host "$numOperatingSystemInstaller OperatingSystemInstallers sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($OperatingSystemInstaller in $selectedOperatingSystemInstaller)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					$scopeName = $secScope.CategoryName
					$scopeID = $secScope.CategoryID
					Write-Host "$cpt) Retrait du scope $scopeName"
					try
					{
						Get-CMOperatingSystemInstaller -Id $OperatingSystemInstaller.PackageID | Remove-CMObjectSecurityScope -id $scopeID -force -ea Stop
						Write-Host "$cpt) Scope retiré"
					}
					catch [Microsoft.ConfigurationManagement.PowerShell.Cmdlets.Utility.RemoveObjectSecurityScope]{
						
					}
					catch [InvalidOperationException]{
						Write-Host "$cpt) Scope non présent"
					}
					catch
					{
...
$choix = [System.Windows.MessageBox]::Show("Est-ce que c'est pour ajouter des security scope?", 'Ajouter ou retirer?', 'YesNoCancel', 'Question')
switch ($choix)
{
	'Yes'{
		. Create-SMSDRIVE
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC8s9dH9bfZY6Df
# ZOcylrrSd9HbiqcBHRrvSM4K+JQ9kqCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# AKBMMBkGCSqGSIb3DQEJAzEMBgorBgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCAT
# 4mULauE3unFxpDt+oIyV4ab6wRbWaIHtE4jQFC28UDANBgkqhkiG9w0BAQEFAASC
# AQBiiWHQjETPhzgSym4Xs41XgiYL7DymHtMASVLpRjuGOasiWPQnLEVcKPlcyc5n
# 9qimIXpSmoDpUopEQ9mYwLjUswfDzbalZNMdCMev+MB1jarv6Tw/yh948+6g5GVu
# QfMkcyj+zrDrOSWMirbyVamJ2hgDEV5ibBOb/MgaMwQ8qyBx5muurmyPCI6wHk4y
# hvCH+CevQqLYyKJTvE644WsdXO7CXaIuRTzokLyBiXYPBrHbXyUD3u2o8Fi+XFgb
# PoCNwv9nmd8HKM6y5P5F12yb0tNxIokUiBG2Aq7AjeyIV7RPBxKo7/CLrxqB5rEl
# r6VPnoSfiGBDPIM2f7Ex5kUxoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMTAxNjI5NDFaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCDdqdm0GgAnh8jLHBIoljaiMN3izCPkNLNCX3gMIt9LdDCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgKAaTSP4WkZWeKAIButi
# hV1Bjyb/BPjQp3EkTDovM8QtyzhcSSTXPHDY5JPSbIimtbGb/lQNuvOZMFZg8fHd
# 9qHC1ybz8aT8mt2yuGrBQtMeFNe9Qi0hoAp+75V8MD8vRDfLy5U+KoJCYXKxeLpg
# SwAlMfscfIgygXe7eENQwuLD047IliyuE2gqB+UdvHFby/Pd8Fg4CAwt5u9/bQd2
# pQg3OO2akxWDKhYSnnkyhDrj8znlSYbkUNRub3J5ms8kbZCccYhXJSZiuR0NKWh1
# c43CSBS9ycfCh8zg7L0pA2C9Lz2y/12Ocgwzz125rek5YElGtSUUBwSykMfFOyyz
# rmqAOFTOKTbG6x0ESnrqC/H7ILhfsfAAwwMcWYfpYZyIloVhRA2PW+F3SzHQ70z/
# pmbqs+Lk7IdbtPJrlCbYbdMlUY+QPjxlQ7J62uwjVpI1xjLVk6xvur4qONdp79mn
# yY7wymKOwXezOVdJHSmy3X2i73LzDAQ8guYqccthOzAb+g==
