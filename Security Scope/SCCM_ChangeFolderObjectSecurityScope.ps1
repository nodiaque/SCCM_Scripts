<#	
	...
	===========================================================================
	 ...
	 Organization: 	STM
	 Filename:     	SCCM_ChangeFolderObjectSecurityScope.ps1
	===========================================================================
	.DESCRIPTION
		Ajoute et retire les security scopes sélectionnés des objets sélectionnés
#>
...
function ajouterScopes
{
	...
	else
	{
		...
		$allFolder = Get-CMFolder
		$selectedFolder = @($allFolder | Select-Object Name, ObjectTypeName, FolderGUID, ObjectType, FolderFlags, ContainerNodeID, SmsProviderObjectPath, isEmpty, ParentContainerNodeID, SearchFolder, SearchString, SourceSite | Out-GridView -passthru -Title "Sélectionner les dossiers à ajouter les scopes.")
		$numFolder = $selectedFolder.count
		if ($numFolder -lt 1)
		{
		...
		{
			write-host "$numFolder dossiers sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($folder in $selectedfolder)
			{
				$folderName = $folder.name
				Write-Host "$cpt) $folderName"
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMFolder -Id $folder.ContainerNodeID | Add-CMObjectSecurityScope -id $scopeID -ea Stop
...
function retirerScopes
{
	...
	else
	{
		Write-Host "Récupération des dossiers"
		$allFolder = Get-CMFolder
		$selectedFolder = @($allFolder | Select-Object Name, ObjectTypeName, FolderGUID, ObjectType, FolderFlags, ContainerNodeID, SmsProviderObjectPath, isEmpty, ParentContainerNodeID, SearchFolder, SearchString, SourceSite | Out-GridView -passthru -Title "Sélectionner les dossiers à retirer les scopes.")
		$numFolder = $selectedFolder.count
		if ($numFolder -lt 1)
		{
			Write-Host "Aucun dossier sélectionné"
		...
		else
		{
			...
			foreach ($folder in $selectedfolder)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMFolder -Id $folder.ContainerNodeID | Remove-CMObjectSecurityScope -id $scopeID -force -ea Stop
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCALmdZkSe1OSEl3
# U3TUHPijrcF6B6VX/COXRsz2CtrxfqCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# hfhDxID7LSDis69rKY5kUAz8V4EnzB4je1BiZf0BtDANBgkqhkiG9w0BAQEFAASC
# AQBOvbam76/52Z5j1k4BjS5xlP7GLfZV5G6vw2amLrip7n3Rw8qGjbNJZdEvyVS2
# 2LIQV13pYMTyjfJ9K5aJozTTxe+8n2x5g/1DwZKcuz8yuLwwdceRSYeL+eYAtfpI
# KnMBce/fzPpLW+a/FRblgb47WNWK39uk4rKAmRgk2HbKIwAYx79ZT+PPtAIU5CBO
# W2J3W4G5jQvMf/LKzpYkXA9UMSjiFQg7LAjiE9mDLU6vEEdf8Mhs0WNDwhAZi8i/
# 2Vakq5/tChLfP+Kn3V3J0AJxAHW/b9ugc/uKmF2/c08ZzVEad2gkoCOGDxX/pixV
# L/RSDP2QsCXjkBfaTtX3NBBNoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMTAxMjQ5NDNaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCDhHa9c4jv3/DMaborW0b04U4dea9XmER2ZojYlEhZYAzCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgAirYHfrhEa4jPTb2asA
# YBjKRclrvcaPSdwCQiemfxXED6bHlJDCZcWsue1n13gBaLAFzPa+FWD5ReG1+E02
# iz9kprblkB2J6eVanUEhpJqmcRb/YL+XNqQqGxT6aVMfuXbeluRgwxPOUYQhPe67
# SaaTs3EFquUVLPOyh+AVmktdRv+wSiKxEnZKAL0fos7d+ouitVzRLUPSAVmdy6Tp
# +pAoxK2qt65MuTsiK9KVkrsvuyYxosW9BnvFtHlcTLXIe5M5G19P/A53Fz7PVugZ
# 8j0BBR35l89ARKRVDG+IToIXtiNIdIytUBc7KUSz8ndl6OucSrdZJXyKLigxZT/k
# F5reXQACWKxZyYbVLAqQXhZn8pKQE9RIT/2bRwD925rwAoL9FV8f66nOX+BU+SRA
# oLwH3NJYq8oAhzT8FAsu0mvvxljHiOojKJwR7VlULkY7CAAujM18KvxTV6BYTYRT
# 94+frQWbN4nY9dIrzWXrv10zUn9hzJ03TcMjYgadcy8M7Q==
