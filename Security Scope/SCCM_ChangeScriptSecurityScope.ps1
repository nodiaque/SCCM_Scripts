<#	
	...
	===========================================================================
	 ...
	 Created on:   	2023-01-24 08:01
	 Created by:   	Jean-Sébastien Frenette
	 Organization: 	STM
	 Filename:     	SCCM_ChangeScriptSecurityScope.ps1
	===========================================================================
	.DESCRIPTION
...
function ajouterScopes
{
	...
	{
		Write-Host "Récupération des scripts"
		$allScripts = Get-CMScript -fast
		$selectedScripts = @($allScripts | Select-Object ScriptName, ScriptGUID | Out-GridView -passthru -Title "Sélectionner les dossiers à ajouter les scopes.")
		$numScript = $selectedScripts.count
		if ($numScript -lt 1)
		{
			Write-Host "Aucun script sélectionné"
		}
		else
		{
			write-host "$numScript scripts sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($cscript in $selectedScripts)
			{
				$cscriptName = $cscript.Scriptname
				Write-Host "$cpt) $cscriptName"
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMScript -scriptguid $cscript.ScriptGUID -Fast | Add-CMObjectSecurityScope -id $scopeID -ea Stop
		...
		else
		{
			...
			foreach ($cscript in $selectedScripts)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMScript -scriptguid $cscript.ScriptGUID -Fast | Remove-CMObjectSecurityScope -id $scopeID -force -ea Stop
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAgjrB30rSZR3IR
# 1x1981omly6gnHose5wqRHz7uQBdnqCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# AKBMMBkGCSqGSIb3DQEJAzEMBgorBgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCAE
# xBGwDvP8BGZrgqKjLaZkpPeTswOipOUDraU9zpFDPTANBgkqhkiG9w0BAQEFAASC
# AQA6na+q+g1LucQ1xdDLICO8xrCgyIvjkorAadvvPLJIw7B9/WNtzafPZciQnKnw
# UKnbGvR/DmiAtYCMJ0B7XJMqnDJCXGiO+P5ZvozE6dE3mKsNGdIznaRE7gPbnLt1
# rFKpNvSwIeJGB+m28prkiB2MQwZRnlCHyuTi7BdPS+TzuJQ2L/38+C0mmaPZR30/
# 018Ja/qjT1Ax8aaKAX6hXODBz4kLrrixUDJwxa/pyKlTIdehGKl9nYcnnU5zNkZz
# 5cN1i0tx61DFV7ZCKPso+M+sFh7Y/jsa06Qse2dhgaadDJPILNgkvsalxw4rJg4Q
# dJLhoosCCA9sNkFAfpx8LZSnoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAxMjQxMzE0MzFaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCAfbF3j6IFEwFRY8iwpeoCC6/jachZ+na3vHLoqm+gtADCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgLg6xuB7nQ2Yoxeyx08/
# 4h5tA0yVRmKtR0rwQVGTm1nexMmto2pEy0YxoGjCFhUyuY253QxenJQr2u1RVWz1
# AlU2wAnir1VIs/uoPnaCH29NSJtsmAVyGhxySVRytWT5UXCHgbD07lgn1mKsgTJO
# ZFE5C6kmoF31B5kquS0XjcXO2W7srNENb5t12KYTmcYgfGC/NED/ntKhtbhybE5/
# T1rZvG4BaOckoMJNM4OoEIpCLdQFIMjfo0Tz3K9RBH38kWiwHChsWxbsp+yXV0M+
# SDIuvkCdAhTOfTQZOu7NyLL7Dfwl1RqPiWyms1G8dBGUQmk95BekSrROUyHHGOHY
# 51t9UzU+bV1l/uUzAvQY0yZH9ccU8Z2VFmjevFNETWksYaPnUApH1bbCplQsXkam
# PrCuhG+vBOIaHQfD+rKiu+1aFwtnB3U9NtsmL3P5tRpi9UfLszPEC5Ht6pL0vqO7
# Jik65wAoGjPJ5diMjdmI/WH0faoYgXGOS7/MNG9phpsKxQ==
