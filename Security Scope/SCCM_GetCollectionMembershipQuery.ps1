...
function collectionsPostes
{
	...
	$allcols = @(Get-CMDeviceCollection)
		Write-Host "Récupération des membership query, ceci prendra un certain temps..."
		$cols = @()
		foreach ($col in $allcols)
		{
			$rules = @(Get-CMDeviceCollectionQueryMembershipRule -CollectionId $col.CollectionID)
		...
		write-host "Affichage des membership query d'ordinateurs"
		
		$cols = @($cols | Select-Object Name, CollectionID, MemberCount, RuleName, QueryID, QueryExpression )
		$rulestomodify = @($cols | Out-GridView -PassThru -Title "Liste des query.")
}
...
function collectionsUtilisateurs
{
	...
	$allcols = @(Get-CMUserCollection)
		...
		foreach ($col in $allcols)
		{
			$rules = @(Get-CMUserCollectionQueryMembershipRule -CollectionId $col.CollectionID)
			foreach ($rule in $rules)
			{
				[hashtable]$excluderuleProp = @{
					Name				  = $col.name
	...
	write-host "Affichage des membership query d'utilisateur"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCgAo9TDxdBSgcP
# Lumz4YGoWNAy2HMKu58/qJ1HUYRVFaCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# AKBMMBkGCSqGSIb3DQEJAzEMBgorBgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCDE
# EsSbpBs9oiav/7MhgSiqmcPI9ziYa2fET0EnaXxH/zANBgkqhkiG9w0BAQEFAASC
# AQBtCIAsyOd/WbXtM3mlmpR7HpW9q1pE50hZoA6ok6IoNt0l/5fE8G/ZRfYxMyH1
# kXbXxx5VJ7/fM6i9D1vjBxqhEB5xADrxKB1UPFxZt7Qmr/X0VuUAGnDj6/i+zOyx
# FB/ouUiXPK3tXG5guS7QUoE8L2xn95A6qvLiBojyon4Aq8ixaIplVaJ7O7uZLrD3
# eT7atjRKIus7XU+wezp3n787/SVrh6L3iXZfBHMaZvaldGkJbB1q0meFDLA9eli+
# yS+ukOUq72mODbg+HwyCo81zfmh1GVlpVjZD2U7/jFLtFpaEPPVHdu4FVvJOST/M
# sDA/BWNhs6utCS24pa0jx5DOoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAxMTMyMDA1MjZaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCDUGgDmXULxVWKhJ1QNpttaeB/fK4H/x9dEniwksk32OTCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgBnnzAm5ibk0XOh01KLC
# zFJ5G1JTA3UTDpTNSgq3gIcC+OaKsnwv5eENuPDQBCXczmk+vhIGPEjOmnhKuPMY
# fmhIqu//CSphhGeYQ9TyWmfn19VZFYBSXhWcS/zEuKCw089FaLCKOYqDUec46Qq0
# nNRHXRXf2fX1WtV5WhEl2JtyObCN4mPfoxfWKPD2ls7CjVsz4+ELkDAWl37f75GL
# 1lo6XOeTx+ov0t9dybTiCyhKQeH5W5+1vJpzfssr+iQ1tiL1heUr66DCzB31E8kP
# SSwtK0ZxOJCx2rFiFOnBiloREQQRZ6+4M4q/U6jw9JslILuL25J0OiGoNTBib9n6
# R6oPYfYBXgDZ/TxyMCCvUvViSYu+ckwRvJgvxDhHJFKul2CJDFsicPnYwPkGA5OL
# lkgfIm54oOvPQ3rmsiC7NthnHhUf8LYA5zrewAYDsWTx9DCCCwy4l3xWzb9PZvaY
# J75GpV1o68H8RqapFeU56plCqbGfzc9aqfksFxpYjA6AJw==
