<#	
	...
	===========================================================================
	 ...
	 Organization: 	STM
	 Filename:     	SCCM_ChangeDriverPackageSecurityScope.ps1
	===========================================================================
	.DESCRIPTION
		Ajoute et retire les security scopes sélectionnés des DriverPackage sélectionnés
#>
...
function ajouterScopes
{
	...
	{
		Write-Host "Récupération des DriverPackage"
		$allDriverPackage = Get-CMDriverPackage -fast
		$selectedDriverPackage = @($allDriverPackage | Select-Object Name, PackageID, SecuredScopeNames, ObjectPath | Out-GridView -passthru -Title "Sélectionner les DriverPackage à ajouter les scopes.")
		$numDriverPackage = $selectedDriverPackage.count
		if ($numDriverPackage -lt 1)
		{
			Write-Host "Aucun DriverPackage sélectionné"
		}
		else
		{
			write-host "$numDriverPackage DriverPackage sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($DriverPackage in $selectedDriverPackage)
			{
				$DriverPackageName = $DriverPackage.Name
				Write-Host "$cpt) $DriverPackageName"
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMDriverPackage -Id $DriverPackage.PackageID -Fast | Add-CMObjectSecurityScope -id $scopeID -ea Stop
...
function retirerScopes
{
	...
	{
		Write-Host "Récupération des DriverPackages"
		$allDriverPackage = Get-CMDriverPackage -fast
		$selectedDriverPackage = @($allDriverPackage | Select-Object Name, PackageID, SecuredScopeNames, ObjectPath | Out-GridView -passthru -Title "Sélectionner les DriverPackages à retirer les scopes.")
		$numDriverPackage = $selectedDriverPackage.count
		if ($numDriverPackage -lt 1)
		{
			Write-Host "Aucun DriverPackage sélectionné"
		...
		{
			write-host "$numDriverPackage DriverPackages sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($DriverPackage in $selectedDriverPackage)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMDriverPackage -Id $DriverPackage.PackageID -Fast | Remove-CMObjectSecurityScope -id $scopeID -force -ea Stop
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDUADJZhdNmJhqK
# yHkSBGlv615Mu8R65OS2aoJyCl9nO6CCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# AKBMMBkGCSqGSIb3DQEJAzEMBgorBgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCCE
# mr0y5TpK7uaw7DQO/9pc+hjkDhlN991dYlyVZJoaJzANBgkqhkiG9w0BAQEFAASC
# AQATnTC6E6auPTCbyaGb64oaHpfE+bDmb65g1eLRvyVmvathZdGsjd+xK0xj1eD7
# +s7wYClVceXaqUTsoW8Im+1Sceruf8f3OzHTkjNctsetvbr7PVk3ess8qTYRn/LT
# +PbCicA8A2gWDYBAJibY0yu1zk141//b7TWYnEekpcy7+eJiK8e6IgZYN2ss4M0i
# ykjx5kXQ1eWNEImeYUP7Ci/Nipkm6fH1du8TDcCb1QB1+JGSqD56jP5XrhsHGXZn
# An58Vj06NxBtw0dMcHOQvr1/IFCzUsO7wT9RlC+2t+DCz8LQdChwXIvZzLAGzBAd
# 4K+hAi8gm0Q3+hAIgLasuF+zoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMTAxNjE3NDZaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCBvsFeECao2l8ce1kRAQvvdRTrv8gMalTJ9zjGbO99BDTCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgCycKehszm7raRuY41pC
# DtFUhWqusxxP+SQS0Y1EMvpOW7ZKl1i3u/9EqffxEm6YIUdeSL5wbjsj/xllPE58
# +rwtjwc4eqBzT8+56CmQk38AMFPQN2uysxW4diI452jgtk/PW8NpuZ0d1ELwPer6
# 3VpdcJs60ICTG0sO/NmMZKFWVPq+leS4JqKNA3QJB3Ix4Ob/NWMC8dxVGTBjIsZV
# ihWLPN1xYYQb8jzKDQ5o62yBz3BHuyLVA5mBtqSjVFUQf4PBykEkp/ZnGWdADWlg
# yDKboCfex+qP0IyyGRtWjRg4mgbuEEd5ij0kNlA/oQT1NwTfXAL1rPTQy6kpKXbD
# 3VYb2FaJgX4tWC6u2amCVgy8cD4F8CF/V2tUeVmywWKE1KMNncxw3nDYb/gy9fo6
# GG/DRfygkx20UQZqPStk7hRIasN9PqYoSD43SqgTgoa2W5Pl1s1f3jJG8ngwY2ZA
# bm41UlvMzoZIC4zGfBCwwF+1GouXYWBQAOQuxguOFODqPg==
