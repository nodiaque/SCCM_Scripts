<#	
	...
	===========================================================================
	 ...
	 Organization: 	STM
	 Filename:     	SCCM_ChangeOSImageSecurityScope.ps1
	===========================================================================
	.DESCRIPTION
		Ajoute et retire les security scopes sélectionnés des OperatingSystemImage sélectionnés
#>
...
function ajouterScopes
{
	...
	{
		Write-Host "Récupération des OperatingSystemImage"
		$allOperatingSystemImage = Get-CMOperatingSystemImage
		$selectedOperatingSystemImage = @($allOperatingSystemImage | Select-Object  Name, PackageID, SecuredScopeNames, Description | Out-GridView -passthru -Title "Sélectionner les OperatingSystemImage à ajouter les scopes.")
		$numOperatingSystemImage = $selectedOperatingSystemImage.count
		if ($numOperatingSystemImage -lt 1)
		{
			Write-Host "Aucun OperatingSystemImage sélectionné"
		}
		else
		{
			write-host "$numOperatingSystemImage OperatingSystemImage sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($OperatingSystemImage in $selectedOperatingSystemImage)
			{
				$OperatingSystemImageName = $OperatingSystemImage.Name
				Write-Host "$cpt) $OperatingSystemImageName"
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMOperatingSystemImage -Id $OperatingSystemImage.PackageID | Add-CMObjectSecurityScope -id $scopeID -ea Stop
...
function retirerScopes
{
	...
	{
		Write-Host "Récupération des OperatingSystemImages"
		$allOperatingSystemImage = Get-CMOperatingSystemImage
		$selectedOperatingSystemImage = @($allOperatingSystemImage | Select-Object Name, PackageID, SecuredScopeNames, Description | Out-GridView -passthru -Title "Sélectionner les OperatingSystemImages à retirer les scopes.")
		$numOperatingSystemImage = $selectedOperatingSystemImage.count
		if ($numOperatingSystemImage -lt 1)
		{
			Write-Host "Aucun OperatingSystemImage sélectionné"
		...
		{
			write-host "$numOperatingSystemImage OperatingSystemImages sélectionnés"
			$cpt = 1
			Write-Host "Début du traitement"
			foreach ($OperatingSystemImage in $selectedOperatingSystemImage)
			{
				...
				foreach ($secScope in $selectedScopes)
				{
					...
					try
					{
						Get-CMOperatingSystemImage -Id $OperatingSystemImage.PackageID | Remove-CMObjectSecurityScope -id $scopeID -force -ea Stop
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA0p+F+WwRjTmCt
# yM0qQ1zJAEi+9YpK+Bj+vM2XaSjeFqCCJQ8wggU9MIIEJaADAgECAhAG4KIaKzBS
...
# AKBMMBkGCSqGSIb3DQEJAzEMBgorBgEEAYI3AgEEMC8GCSqGSIb3DQEJBDEiBCBz
# du81KbnH8G+Wa4uDacXfs2PJ/RZ8ucyubUYIskTIPTANBgkqhkiG9w0BAQEFAASC
# AQCT8VN3w30rHh/eRfIBh2RgJddmzUa63qQrTl4K/GhFux4+f8fu+q5lwJx/43AN
# aVStXz67WYUXx3DfYyKsLe2jrzmSwgNEdsESRwPEaN3DiRAOGwCKSjZMjWWmRl4q
# KOezOqA2n9KrrYuhYRvdcfUtDL1Itlu/6QrFXsEKtdPx51gpBdpylLfPNqGF70by
# LwclduJp7lT8VN0GXTuMs64ToD2jyppDgL3k3TkxSPwFKxTdTxd+XY+QJVL0E+JL
# 96KBlwiYzd8fOJdDBk/Yp8EsDD5iTTTA8LcmTaQhKdK6HBleeY9PxVUhZ/vv+MvS
# 6UNjsopVHuqj8Fo0xIzCufmmoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEB
...
# CwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMTAxNjI0NDNaMCsGCSqG
# SIb3DQEJNDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3
# DQEJBDEiBCAZpaFuz6M5gx4dtCuj7iUYEMeEkHTLcKE8i1115PzPjDCBpAYLKoZI
...
# SJA9woq8p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgIyyGpNQ3pn3zbnJal5G
# sFTtLgfcxk0B+ynGl4qnBXGCr/c0sZEuF4Wn3qIW/NkPA+N12SpZY74ikrRCbQsZ
# Hd2yDUdIAnI3F9G9mDi2D1TIpPhWbkH5hmY9mw5HxGJDIEK7w8W9LmZS1L5qqhK6
# mTv4V8EYettivhQpn4pzIjsAD0bgtIAFdUMtfZRk4BJpYNe/HOgAg6FLvWUXpB43
# NTbmUkyOqM6f4yuLGhaVg3iLcv1PnQWfqobjQuEMVuLG/ucBzQb7GU8WukagHJqZ
# FzubGn00hr5FAD3nv3APqaRZjZtRK7yLzQqlb0NIL4D4gMo8FQnyLvVyQHKlRTNO
# ujG//lwDu4LYSKyd8qF/hmaahBhWoS5/MZyy6Wrak3tFKldMlEOSPwZ4ER7Lk9nH
# 6ADNW7vEBpYJJhDyDaB17gapwJeKWs5w+Asmk9BSrmdkdjcgGyXzSCDjNWuaclpp
# gphO/A3SWPOrU/g2lc8elOSD6thVMm5G8cS1HDAJHKQYeQ==
